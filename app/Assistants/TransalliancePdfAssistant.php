...
class TransalliancePdfAssistant extends PdfClient
{
    ...
    public function processLines(array $lines, ?string $attachment_filename = null)
    {
        ...
        foreach ($lines as $index => $item) {
            ...
            if (Str::startsWith($item, 'REF.:') && !isset($order_reference)) {
                $ref_li = $index;
                $order_reference = trim(str_replace('REF.:', '', $item));
            } else if ($item === 'TRANSAVIS UAB') {
                $customer_start = $index + 2;
        ...
        $customer_location_data = array_slice($lines, $customer_start, $ref_li - $customer_start);
        ...
        $destination_locations = $this->extractLocationsForDelivery(
    ...
    private function extractCustomerAddress(array $customerData): array
    {
        ...
        foreach ($customerData as $index => $item) {
            ...
            if (Str::startsWith($item, 'Contact:')) {
                $contact_person = $customerData[$index + 1];
            } else if (Str::startsWith($item, 'Tel :')) {

                $telephone_li = $index;

            } else if ($item === 'VAT NUM:') {

                $vat_code = $customerData[$index + 1];
            } else if ($item === 'E-mail :') {
    ...
    private function extractLocations(array $loadingData): array
    {
        ...
        foreach ($loadingData as $index => $item) {
            if (Str::contains($item, 'ON:')) {
                $on_li = $index;
            } else if ($item === 'Payment terms :') {
                $payment_term_li = $index;
        ...
        $onBlock = array_slice($loadingData, $on_li + 2, $payment_term_li - $on_li - 3);
        $company_address = $this->parseAddressBlock($onBlock);
        $company_address['contact_person'] = $contact_person;
        $loading_locations = [
            [
    ...
    private function extractLocationsForDelivery(array $loadingData): array
    {
        $company_address = [];
        $time_interval = [];
        foreach ($loadingData as $index => $item) {
        ...
        $onBlock = array_slice($loadingData, 5, $lm_li - $on_li - 7);
        $company_address = $this->parseAddressBlock($onBlock);
        $company_address['contact_person'] = $contact_person;
        $loading_locations = [
            [
    ...
    private function parseAddressBlock(array $lines): array
    {
        ...
        if (
            preg_match(
                '/^(?:(?<country>[A-Z]{2})[- ]?)?(?<postal>[A-Z0-9\- ]{3,10})\s+(?<city>.+)$/iu',
                ltrim($lastPart, '- '),
                $m
            )
        ) {
